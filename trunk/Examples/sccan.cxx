  ...
  for( unsigned int j = 0; j < image_fn_list.size(); j++ )
    ...
    for( unsigned int k = 0; k < vec_fn_list.size(); k++ )
      ...
      for(  mIter.GoToBegin(), mIter2.GoToBegin(); !mIter.IsAtEnd() && !mIter2.IsAtEnd(); ++mIter, ++mIter2 )
        ...
        if( mIter2.Get() > 0 )
          {
          dotCounter += mIter2.Get(); dotTotal += mIter.Get() * mIter2.Get();
          }
        }
              ...
              unsigned int iterct = 20 )
{
  ...
  if( outputOption )
    ...
    std::string post = std::string("View1vec");
    antscout << " have_p_mask " << have_p_mask << std::endl;
    WriteVariatesToSpatialImage<ImageType, Scalar>( filename, post,
                                                    sccanobj->GetVariatesP(), mask1,
                                                    sccanobj->GetMatrixP(), have_p_mask );
...
int sccan( itk::ants::CommandLineParser *parser )
{
  ...
  if( !permoption || permoption->GetNumberOfValues() == 0 )
    ...
    {
    permct = parser->Convert<unsigned int>( permoption->GetValue() );
    }
  ...
  if( !clust_option || clust_option->GetNumberOfValues() == 0 )
    ...
    {
    p_cluster_thresh = parser->Convert<unsigned int>( clust_option->GetValue() );
    }
  ...
  if( !clust_option || clust_option->GetNumberOfValues() == 0 )
    ...
    {
    q_cluster_thresh = parser->Convert<unsigned int>( clust_option->GetValue() );
    }
  ...
  if( !eigen_option || eigen_option->GetNumberOfValues() == 0 )
    ...
    {
    eigen_imp = parser->Convert<bool>( eigen_option->GetValue() );
    }
